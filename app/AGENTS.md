...
- The Phase 3 metric contract lives in `src/domain/metrics/metricCatalog.js`; database observations, page projections, units, cadence, quality, and default display must stay aligned with it.
- Source release decisions live in `src/domain/metrics/sourcePolicy.js` and `DATA_SOURCE_REVIEW.md`. Unknown, blocked, or approval-required-without-a-gate sources cannot enter a new public projection.
- GitHub Actions separates collection/persistence, public projection, and build/deploy with reusable workflows and passes data between stages as short-lived artifacts.
- The homepage dashboard prioritizes a dense, broad scan: render one metric per compact cell, group related metrics inside a larger category panel, and show only the metric name, current value, daily change, and weekly change in the grid. Keep methodology, provenance, freshness, and limitations in the shared data-trust area instead of repeating notes inside every cell.
- Keep the existing homepage metrics while adding every metric represented in the macro-liquidity and U.S.-equity overview strips. A metric whose reviewed source is unavailable must keep its cell and show `N/A`; never bypass source policy by copying a blocked legacy cache into the dashboard projection.
- The device-local dashboard layout editor is a dismissible side panel. It must stay keyboard accessible, default to closed so the metric grid gets the full canvas, and continue to persist only visibility and ordering on the current device.
