...
- The market-clock page must consume backend-generated absolute status intervals from reviewed official NYSE, KRX, and SSE calendars. Holiday, weekend, early-close, and next-transition logic belongs in the backend cache; the frontend only selects the current interval and renders its countdown.
