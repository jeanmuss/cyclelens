...
首页默认布局共有 **8 个分组、25 个指标单元格**。其中 13 个是第一版首页依赖，12 个是本批为“美股大盘”和“宏观流动性”概况补齐的新依赖。仓库内离线 LKG 基线为 **11 个可发布、14 个显示 N/A**；`Deploy GitHub Pages #20` 的完整 CMC 刷新又合规发布了 USDT、USDC 流通市值，生产终态为 **13 个可发布、12 个显示 N/A**。公开 `dashboard` projection 另保留一个非默认单元格依赖 `stablecoin.usdt.depegBps`，因此线上 projection 有 14 个指标，不能把它误算进 25 个默认单元格。可用数会随已批准来源的新观测变化，但 25 个单元格及其 N/A 退化行为保持稳定。
